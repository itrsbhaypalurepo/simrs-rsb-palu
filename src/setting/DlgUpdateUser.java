...
public class DlgUpdateUser extends javax.swing.JDialog {
    ...
    private boolean penyakit=false,obat_penyakit=false,dokter=false,jadwal_praktek=false,petugas=false,pasien=false,registrasi=false,tindakan_ralan=false,kamar_inap=false,
            ...
            catatan_observasi_restrain_nonfarma=false,catatan_observasi_ventilator=false,catatan_anestesi_sedasi=false,skrining_puma=false,satu_sehat_kirim_rencana_kontrol=false,satu_sehat_kirim_questionnaire_request=false,satu_sehat_kirim_careplan=false;
    ...
    */
    ...
    private void tampil(String user) {  
        ...
        try{    
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.catatan_observasi_ventilator,user.catatan_anestesi_sedasi,user.skrining_puma,user.satu_sehat_kirim_rencana_kontrol,user.satu_sehat_kirim_questionnaire_request,user.satu_sehat_kirim_careplan from user where user.id_user=AES_ENCRYPT(?,'nur')");
            try {
                ps.setString(1,user);
                rs=ps.executeQuery();
                if(rs.next()){
                    ...
                    catatan_observasi_restrain_nonfarma=rs.getBoolean("catatan_observasi_restrain_nonfarma");catatan_observasi_ventilator=rs.getBoolean("catatan_observasi_ventilator");catatan_anestesi_sedasi=rs.getBoolean("catatan_anestesi_sedasi");skrining_puma=rs.getBoolean("skrining_puma");satu_sehat_kirim_rencana_kontrol=rs.getBoolean("satu_sehat_kirim_rencana_kontrol");satu_sehat_kirim_questionnaire_request=rs.getBoolean("satu_sehat_kirim_questionnaire_request");satu_sehat_kirim_careplan=rs.getBoolean("satu_sehat_kirim_careplan");
                    setTampil();
                }       
    ...
    private void setTampil(){
        ...
        if("[L]Kirim Rencana Kontrol Satu Sehat".toLowerCase().contains(TCari.getText().toLowerCase())){
            tabMode.addRow(new Object[]{false,"[L]Kirim Rencana Kontrol Satu Sehat",satu_sehat_kirim_rencana_kontrol});
        }
        
        if("[L]Kirim Questionnarie Response Satu Sehat".toLowerCase().contains(TCari.getText().toLowerCase())){
            tabMode.addRow(new Object[]{false,"[L]Kirim Questionnarie Response Satu Sehat",satu_sehat_kirim_questionnaire_request});
        }
    ...
    private void Simpan2() {
        for(i=0;i<tbUser.getRowCount();i++){ 
            ...
            if("[L]Kirim Rencana Kontrol Satu Sehat".equals(tbUser.getValueAt(i,1).toString())){
                Sequel.mengedit("user","id_user=AES_ENCRYPT('"+TKd.getText()+"','nur')","satu_sehat_kirim_rencana_kontrol='"+tbUser.getValueAt(i,2).toString()+"'");
            }
            
            if("[L]Kirim Questionnarie Response Satu Sehat".equals(tbUser.getValueAt(i,1).toString())){
                Sequel.mengedit("user","id_user=AES_ENCRYPT('"+TKd.getText()+"','nur')","satu_sehat_kirim_questionnaire_request='"+tbUser.getValueAt(i,2).toString()+"'");
            }
