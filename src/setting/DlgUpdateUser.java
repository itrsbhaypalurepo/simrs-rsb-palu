...
public class DlgUpdateUser extends javax.swing.JDialog {
    ...
    private boolean penyakit=false,obat_penyakit=false,dokter=false,jadwal_praktek=false,petugas=false,pasien=false,registrasi=false,tindakan_ralan=false,kamar_inap=false,
            ...
            catatan_observasi_restrain_nonfarma=false,catatan_observasi_ventilator=false,catatan_anestesi_sedasi=false,satu_sehat_kirim_rencana_kontrol=false;
    ...
    private void tampil(String user) {  
        ...
        try{    
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.catatan_observasi_ventilator,user.catatan_anestesi_sedasi,user.satu_sehat_kirim_rencana_kontrol from user where user.id_user=AES_ENCRYPT(?,'nur')");
            try {
                ps.setString(1,user);
                rs=ps.executeQuery();
                if(rs.next()){
                    ...
                    catatan_observasi_restrain_nonfarma=rs.getBoolean("catatan_observasi_restrain_nonfarma");catatan_observasi_ventilator=rs.getBoolean("catatan_observasi_ventilator");catatan_anestesi_sedasi=rs.getBoolean("catatan_anestesi_sedasi");satu_sehat_kirim_rencana_kontrol=rs.getBoolean("satu_sehat_kirim_rencana_kontrol");
                    setTampil();
                }       
